...
// PDF generation is handled by the frontend using Playwright for better quality
...
async function processBookGeneration(job: Awaited<ReturnType<typeof getJobWithDetails>>): Promise<void> {
  ...
  // PDF generation is handled by the frontend using Playwright for better quality
  // The frontend will trigger PDF generation when user views/downloads the book
  console.log(`[Worker] All illustrations complete. PDF will be generated by frontend on demand.`);
}
